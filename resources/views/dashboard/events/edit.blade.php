...
<section class="my-2">
    <div class="container-fluid">
        <form action="{{ route( 'anume.dashboard.events.update', [ 'id' => $event->id ] ) }}" class="row" id="editEventForm" method="POST" enctype="multipart/form-data">
         @csrf
         <div class="col-lg-3">
           ...
           <div class="card alert alert-light border border-info p-0">
             <div class="card-body p-2">
               ...
               <input type="date" name="start_date" class="form-control @error('start_date') {{ 'is-invalid' }} @enderror" value="{{ old('start_date') ?? $event->start_date }}">
               <small class="text-danger">{{ $errors->first('start_date') }}</small><hr />

               <label for="endDate">End Date</label>
               <input type="date" name="end_date" class="form-control @error('end_date') {{ 'is-invalid' }} @enderror" value="{{ old('end_date') ?? $event->end_date }}">
               <small class="text-danger">{{ $errors->first('end_date') }}</small>

               <div class="mt-3 row">
                 <div class="col-lg-6">
                   <label for="startTime">Start Time</label>
                   <input type="time" name="start_time" class="form-control @error('start_time') {{ 'is-invalid' }} @enderror" value="{{ old('start_time') ?? $event->start_time }}">
                 </div>
                 <div class="col-lg-6">
           ...
           <div class="card alert alert-light border border-light p-0">
             <div class="card-body p-2">
              ...
              <textarea name="extract_text" id="extractText" cols="30" rows="5" class="mt-2 form-control" placeholder="Extract Text">{{ old('extract_text') ?? $event->extract_text }}</textarea>
         ...
         <div class="col-lg-6">
           ...
           <div class="card shadow-sm border border-primary">
             <div class="card-body p-0">
              <textarea name="title" id="" cols="3" rows="2" class="form-control " placeholder="Event Title">{{ old('title') ?? $event->title }}</textarea>
             </div>
           </div>
           ...
           <div class="card shadow-sm border border-primary mt-2">
            <div class="card-body p-0">
             <input type="text" name="slug" class="form-control" placeholder="Event Slug" value="{{ old('slug') ?? $event->slug }}" />
            </div>
          </div>
          ...
          <small class="text-danger">{{ $errors->first('slug') }}</small>
           ...
           <div class="card shadow-sm alert p-0 alert-primary my-2 border border-primary">
            <div class="card-body p-0">
             <textarea name="content" id="editor" cols="3" rows="10" class="form-control" placeholder="Event Content">{{ old('content') ?? $event->content }}</textarea>
            </div>
          </div>
